...
label_list = ["FRI", "FRII", "Compact", "Bent"]
...
with h5py.File('data/galaxy_data_h5.h5', 'r') as F:
    for key in F.keys():
        ...
        if split_entry == "train":
            print(key, F[key], F[key].keys())
            data_entry = F[key + "/Img"]
            label_entry = F[key + "/Label_literature"]
            d = np.array(data_entry)
            labels.append(np.array(label_entry))
            images.append(d)

            print(key, np.array(label_entry))
...
x_train_clean = x_train #format_galaxies(x_train, threshold = 0.1, min_size = 100, margin = 2)
x_test_clean  = x_test #format_galaxies(x_test, threshold = 0.1, min_size = 100, margin = 2)
